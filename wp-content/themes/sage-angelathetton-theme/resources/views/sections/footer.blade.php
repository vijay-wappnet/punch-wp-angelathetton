...
if (!$aria_label) {
    $aria_label = __('Home', 'sage');
                    ...
                    @if ($footer_logo)
                        @if ($link_url)
                            <a href="{{ esc_url($link_url) }}" class="footer-logo-link"
                                aria-label="{{ esc_attr($aria_label) }}" {!! $link_target ? 'target="' . esc_attr($link_target) . '"' : '' !!}
                                {!! $ga_label ? 'data-event="' . esc_attr($ga_label) . '"' : '' !!}>
                                <img src="{{ esc_url($footer_logo['url']) }}"
                                    alt="{{ esc_attr($footer_logo['alt']) }}" class="img-fluid">
                            </a>
                        @else
                ...
                <div class="col-12 col-md-5 footer-links">
                    {{-- Mobile-only sitemap toggle --}}
                    <div class="footer-sitemap-toggle">
                        <span>SITE MAP</span>
                        <img src="{{ Vite::asset('resources/images/bottom_arrow.svg') }}" alt="chevron down" class="sitemap-chevron">
                    </div>

                    <div class="footer-sitemap-content">
                        @if (has_nav_menu('footer_main_navigation'))
                            <nav class="footer-main-nav"
                                aria-label="{{ wp_get_nav_menu_name('footer_main_navigation') }}">
                                {!! wp_nav_menu([
                    ...
                    <div class="input-group">
                        ...
                        <form class="search-results-form" method="GET"
                                        action="@php echo esc_url(home_url('/search-results/')); @endphp">
                                                    <input type="text" name="search-field" class="form-control footer-search-input" placeholder="Search"
                                                        aria-label="Search">
                        </form>
                    </div>
                </div>
