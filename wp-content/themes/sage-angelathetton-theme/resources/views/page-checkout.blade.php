...
@section('content')
  ...
  <div class="container checkout-page">
    ...
    @if (! $checkout->is_registration_enabled() && $checkout->is_registration_required() && ! is_user_logged_in())
      <p class="woocommerce-info">{!! apply_filters('woocommerce_checkout_must_be_logged_in_message', __('You must be logged in to checkout.', 'woocommerce')) !!}</p>
    @else
        ...
        <div class="row checkout-layout">
          <div class="col-lg-6 col-12 checkout-customer-column">
            <div class="checkout-tabs" role="tablist" aria-label="{{ __('Checkout details tabs', 'sage') }}">
              <button type="button" class="checkout-tabs__button active" data-tab-target="billing-details" aria-expanded="true">
          ...
          <div class="col-lg-6 col-12 order-summary-column">
            ...
            <div id="order_review" class="woocommerce-checkout-review-order order-summary">
              <button type="button" class="order-summary-header" aria-expanded="true" aria-controls="order-summary-content">
                ...
                <img src="{{ \Illuminate\Support\Facades\Vite::asset('resources/images/bottom_arrow.svg') }}" alt="{{ __('Toggle order summary', 'sage') }}" class="icon">
              </button>

              <div id="order-summary-content" class="order-summary-body">
                <div id="custom-checkout-products" class="order-summary-products" data-nonce="{{ wp_create_nonce('checkout_cart_nonce') }}">
                    ...
                    <div class="order-summary-product" data-cart-item-key="{{ $cart_item_key }}">
                      ...
                      <div class="order-summary-product__actions">
                        ...
                        <div class="quantity-input">
                          <button type="button" class="quantity-input__btn minus" aria-label="{{ __('Decrease quantity', 'woocommerce') }}">-</button>
                          <input type="number"
                            class="qty"
                          ...
                          <button type="button" class="quantity-input__btn plus" aria-label="{{ __('Increase quantity', 'woocommerce') }}">+</button>
                        </div>
                      </div>
                    </div>
                  @endforeach
            ...
            <div id="payment" class="payment-methods woocommerce-checkout-payment">
                  ...
                  <ul class="wc_payment_methods payment_methods methods">
                    @foreach ($available_gateways as $gateway)
                      <li class="wc_payment_method payment_method_{{ $gateway->id }}">
                        <input id="payment_method_{{ $gateway->id }}" type="radio" class="input-radio" name="payment_method" value="{{ $gateway->id }}" @checked($gateway->chosen)>
                        <label for="payment_method_{{ $gateway->id }}">{!! $gateway->get_title() !!}</label>

                        @if ($gateway->has_fields() || $gateway->get_description())
                          <div class="payment_box payment_method_{{ $gateway->id }}" @if (! $gateway->chosen) style="display:none;" @endif>
                            @php
              ...
              <div class="place-order custom-place-order--from-template--override">
                ...
                <button type="submit" class="button alt" name="woocommerce_checkout_place_order" id="place_order" value="{{ esc_attr($order_button_text) }}" data-value="{{ esc_attr($order_button_text) }}">
